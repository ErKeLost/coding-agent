...
export const stopSandbox = createTool({
  id: 'stopSandbox',
  description: 'Stop an existing Daytona sandbox. If sandboxId is omitted, reuse the current thread or request sandbox when available.',
  ...
  execute: async (inputData, context) => {
    ...
    if (!sandboxId) {
      throw new Error('No sandbox available to stop. Provide sandboxId or create a sandbox first.');
    }

    const sandbox = await getSandbox(sandboxId);
    await sandbox.stop(inputData.timeoutSeconds ?? 60);
